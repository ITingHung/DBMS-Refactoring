...
class AggregateWindow(Window):
    ...
    def initial_gui(self):
        ...
        # Show total record of the selected table
        self.total_record(self.table_combo.get())
        self.display_result(
            f'SELECT * FROM {self.table_combo.get()}')  # Display record

    # Condition field
    def display_condition(self):
        # Label
        self.condition_label = tk.Label(
            self.window, text='Condition', font=self.font_style)
        ...
        for i in range(len(self.table_columns)):
            ...
            self.cursor.execute(
                f'SELECT DISTINCT {self.table_columns[i][0]} FROM {self.table_combo.get()}')
            self.column_combo.append(ttk.Combobox(
                self.window, values=self.cursor.fetchall(), font=self.font_style))
            self.column_combo[i].place(
                x=20+190*(i), y=160, height=30, width=190)

    # Show the number of total record in selected table
    def total_record(self, table_name):
        self.record_label = tk.Label(
            self.window, text='', anchor='w', font=self.font_style)
    ...
    def send_query(self):
        ...
        for i in range(len(self.column_combo)):
            if self.column_combo[i].get():
                ...
                self.cursor.execute(
                    f'SELECT COUNT({self.table_columns[i][0]}) FROM {self.table_combo.get()} WHERE {self.condition}')
        count_condition = self.cursor.fetchone()[0]
        self.count_result = tk.Label(
            self.window, text=f'Record: {count_condition}', background='gray80', font=self.font_style)
        ...
        for i in range(len(self.table_columns)):
            ...
            if self.table_columns[i][1] == 'int':
                for agg in range(len(aggregate_list)):
                    self.cursor.execute(
                        f'SELECT {aggregate_list[agg]}({self.table_columns[i][0]}) FROM {self.table_combo.get()}')
                    agg_total = self.cursor.fetchone()[0]
                    self.cursor.execute(
                        f'SELECT {aggregate_list[agg]}({self.table_columns[i][0]}) FROM {self.table_combo.get()} WHERE {self.condition}')
                    ...
                    self.agg_result[agg].place(
                        x=20+190*i, y=240+30*agg, height=30, width=190)
        # Display record
        self.display_result(
            f'SELECT * FROM {self.table_combo.get()} WHERE {self.condition}')
